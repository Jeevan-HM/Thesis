...
def main():
    ...
    for p in csv_files:
        ...
        try:
            ...
            pouch_cols = [
                c for c in df.columns if "Measured_pressure_Segment_1_pouch" in c
            ][:5]
            if len(pouch_cols) < 5:
                continue
            ...
            u_cols = [
                c
                for c in df.columns
                if "Desired_pressure_segment" in c and c != "Desired_pressure_segment_1"
            ]
            if len(u_cols) < 3:
                continue
            ...
            if "theta" not in df.columns:
                ...
                df["theta"] = (
                    np.arcsin(np.clip(2 * (qw * qy - qz * qx), -1.0, 1.0)) * 180 / np.pi
                )
            ...
            response_div = (
                np.std(S_stds) / np.mean(S_stds) if np.mean(S_stds) > 0 else 0
            )
    ...
    out_summary = os.path.join(args.out_dir, "prc_analysis_summary.csv")
    out_figures = os.path.join(args.out_dir, "prc_analysis_figures.pdf")
    ...
    print(f"Analysis complete. Wrote: {out_summary}, {out_figures}")
