    ...
    def th_pd_gen(self):
        ...
        try:
            if self.flag_reset==1:
                ...
                array = [seg_5,seg_6,seg_7,seg_8, seg_4]
                #array = [seg_7,seg_8]
            ...
            while (time()-self.t0_on_glob < self.trailDuriation):
                ...
                try:
                    ...
                    for i in range(4):
                        ...
                        self.pres_single_ramp_response(col,freq,upper_bound,lower_bound)
                        print(i)
                        self.trial_start_reset = 1
...
## Sine Wave
    ...
    def pres_single_ramp_response(self,col,freq,upper_bound,lower_bound):
        #print("ramping")
        t = time() - self.t0_on_trial # range from 0
        total = (2*np.pi)/(2*np.pi*freq)
        while (self.th1_flag and self.th2_flag and (t <= total)):
            ...
            try:
                ...
                for i in range(len(self.NArs)):
                    self.pd_array_1[i] = ((upper_bound+lower_bound)/2) + (((upper_bound-lower_bound)/2)*math.sin(2*np.pi*freq*t))
