...
class pc_client(object):
    ...
    def th_pd_gen(self):
        ...
        try:
            if self.flag_reset==1:
                ...
                #step signal of 3 psi to segs 1 and 3
                seg_1 = 3
                seg_2 = 1
                seg_3 = 3
                seg_4 = 3
            ...
            while (time()-self.t0_on_glob < self.trailDuriation):
                time_check = time()-self.t0_on_glob
                #print(time_check)
                try:
                    #print("heheheh")
                    #if self.flag_use_mocap == True:
    ...
    def th_data_exchange_high(self):# thread config of read data from mocap and send packed msg to record file.
        ...
        #file_name='data_collect_tues_oldsetup_4.txt'
        ...
        while self.run_event.is_set() and self.th3_flag:
            try:
                ...
                self.timestamp = np.array([round((time()-t0),6)])
                #print(timestamp)
                self.arr_comb_record=np.concatenate((self.timestamp,self.pd_array_1, self.pm_array_1, self.array3setswithrotation), axis=None)
                ...
                #self.arr_comb_record = np.concatenate((self.timestamp,self.arr_comb_record))

                if self.z == 0:
                    self.new_arr_comb_record = self.arr_comb_record
                    #self.new1_arr_comb_record = self.arr_comb_record
                    self.z = self.z + 1
                else:
                    #self.z = 2
                    self.new_arr_comb_record = np.vstack((self.new_arr_comb_record,self.arr_comb_record))
                    #self.new_arr_comb_record = self.final_arr_comb_record


                #print(self.arr_comb_record)
                #msg=self.new_arr_comb_record
                #print(msg[0])
                

                if self.flag_reset==0:
                    self.send_zipped_socket1(self.new_arr_comb_record)
    ...
    def record_data():
        ...
        file_name='data_collect_test6.txt'
        with open(file_name,'w+') as data_file:
            print("Data Recording")
            csvwriter = csv.writer(data_file, lineterminator = '\n')
            csvwriter = writerows(msg)
