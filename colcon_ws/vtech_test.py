...
class pc_client(object):
    ...
    def th_pd_gen(self):
        ...
        try:
            if self.flag_reset==1:
                ...
                array = [seg_1,seg_2,seg_3,seg_4]

                self.t0_on_trial = time()
                self.pres_single_step_response((array),15)
    ...
    def th_data_exchange_high(self):# thread config of read data from mocap and send packed msg to record file.
        ...
        #file_name='vtech_new_test5.txt'
        file_name='sensor_calb_1.txt'
        start_time = time()
        received_count = 0
        self.comm_rate = []
        ...
        with open(file_name,'w+') as data_file:
            ...
            while self.run_event.is_set() and self.th3_flag:
                try:
                    ...
                    self.arr_comb_record=np.concatenate((self.pd_array_1, self.pm_array_1, self.offset, self.sums, self.array3setswithrotation), axis=None)
                    #self.arr_comb_record = np.concatenate((self.arr_comb_record[:24],self.arr_comb_record[29:]))
                    #self.arr_comb_record = np.concatenate((self.arr_comb_record[:21],self.arr_comb_record[23:]))
                    #self.arr_comb_record = np.concatenate((self.arr_comb_record[:18],self.arr_comb_record[20:]))
                    #self.arr_comb_record = np.concatenate((self.arr_comb_record[:15],self.arr_comb_record[17:]))
                    #self.arr_comb_record = np.concatenate((self.arr_comb_record[:6],self.arr_comb_record[11:]))
                    #self.arr_comb_record = np.concatenate((timestamp,self.arr_comb_record))
...
###############Arduino Sockets###########################
    ...
    def ard_socket(self, pd_array, client, flags=0):
        ...
        for i in range(0,len(data)):
            ...
            pm = ((60.0 - 0.0) * (pressure_volt_1 - (0.1 * 5.0))) / (0.8 * 5.0)

            if self.counter < 1000:
                self.counter = self.counter + 1

                if client == self.client_sockets[0]:
                    self.sums[0] = self.sums[0] + (pd_array - data[0])
                if client == self.client_sockets[1]:
                    self.sums[1] = self.sums[1] + (pd_array - data[0])
                    self.sums[2] = self.sums[2] + (pd_array - data[1])
                    self.sums[3] = self.sums[3] + (pd_array - data[2])
                if client == self.client_sockets[2]:
                    self.sums[4] = self.sums[4] + (pd_array - data[0])
                    self.sums[5] = self.sums[5] + (pd_array - data[1])
                    self.sums[6] = self.sums[6] + (pd_array - data[2])
                if client == self.client_sockets[3]:
                    self.sums[7] = self.sums[7] + (pd_array - data[0])

            if self.counter == 1000:
                for j in range(0,8):
                    self.offset[j] = self.sums[j]/self.counter
