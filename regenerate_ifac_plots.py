...
# Notes
# -----
# Panel 4(b) in the final manuscript uses the *signal amplitude* view
# (mean pouch standard deviation). Pass ``--panel4b-mode corr`` if you want the
# inter-pouch correlation variant instead.
...
# aggregate metrics remain unchanged. All plot text is configured with a
# Times New Roman-compatible serif stack for manuscript consistency.
# """
...
# RIDGE_ALPHA = 0.01
# TRIM_SEC = 10.0
# PRED_HORIZON_SEC = 0.0
# MEMORY_K = 40
# MAX_XCORR_LAG_SEC = 15.0

# # ── Font-size configuration (edit here to resize all figures at once) ────────
# FONT_SIZE_TITLE = 14  # axes / subplot titles
# FONT_SIZE_LABEL = 14  # x- and y-axis labels
# FONT_SIZE_TICK = 12  # tick-mark labels
# FONT_SIZE_LEGEND = 11  # legend text
# FONT_SIZE_ANNOTATION = 14  # annotations and inset text boxes
...
#         choices=("sigma", "corr"),
#         default="sigma",
#         help="Use mean pouch sigma (matches the final manuscript figure) or inter-pouch correlation.",
#     )
#     parser.add_argument(
#         "--dpi",
#         type=int,
...
# def mean_pairwise_ccc(sensors: np.ndarray) -> float:
#     """Return the mean pairwise concordance correlation coefficient across pouches."""
#     vals = []
#     for i in range(sensors.shape[1]):
#         x = sensors[:, i]
#         mux = float(np.mean(x))
#         varx = float(np.var(x, ddof=1))
#         for j in range(i + 1, sensors.shape[1]):
#             y = sensors[:, j]
#             muy = float(np.mean(y))
...
#                 "mean_ccc": mean_pairwise_ccc(trial.sensors),
...
#         and t.meta.waveform == "triangular"
...
#     # Panel (a): Parallel representative trace
...
#     # Panel (b): Coupled representative trace
...
#     # Panel (c): NMSE box plot (raw sensors, original params)
#     box_order = [
#         ("axial", "coupled"),
#         ("axial", "parallel"),
#         ("circular", "coupled"),
...
#         ("triangular", "parallel"),
#     ]
#     box_positions = [1, 2.5, 4.5, 6, 8, 9.5]
#     box_data = [
#         metrics.loc[
...
#     )
#     ax3.set_ylabel("NMSE")
#     ax3.set_title("(c) 1 s-ahead prediction NMSE by waveform and topology")

#     # Panel (d): DDI (raw sensors, original params)
#     mc_data = [
...
#             linewidth=1.4,
#             label=f"P{i + 1}" if i < 5 else None,
#         )
#         ax4.plot(
#             parallel_lag,
#             parallel_xcorr[i],
...
#             linestyle="--",
#         )
#     # Add style legend: solid = coupled, dashed = parallel
#     from matplotlib.lines import Line2D

#     style_handles = [
#         Line2D([0], [0], color="gray", linewidth=1.4, linestyle="-", label="Coupled"),
#         Line2D([0], [0], color="gray", linewidth=1.4, linestyle="--", label="Parallel"),
#     ]
#     leg1 = ax4.legend(
...
#         ax2.set_title("(b) Mean pouch standard deviation")
#         ax2.set_ylabel(r"Mean pouch $\sigma$ (PSI)")
#     else:
#         ax2.set_title("(b) Mean inter-pouch correlation")
...
#             xs, ys, yerr=[yerr_lo, yerr_hi], marker="o", label=topo.capitalize()
#         )
#     ax3.set_title("(c) Median NMSE vs pre-inflation")
...
#                 "mean_ccc",
#             ].to_numpy()
#             med, _, _ = quantile_errorbars(vals)
#             xs.append(pre)
#             ys.append(med)
#         ax4.plot(xs, ys, marker="o", label=topo.capitalize())
#     ax4.set_title("(d) Mean CCC vs pre-inflation")
#     ax4.set_xlabel("Pre-inflation (PSI)")
#     ax4.set_ylabel("Mean CCC")
...
#     ms = np.array(m_values, dtype=int)
#     best_fixed = np.array(fixed_medians)
#     marginal_gain = best_fixed[:-1] - best_fixed[1:]
#     step_labels = ["1->2", "2->3", "3->4", "4->5"]
#     total_gain = best_fixed[0] - best_fixed[-1]
#     recovered_2 = marginal_gain[0] / total_gain if total_gain > 1e-12 else 0.0
#     recovered_3 = (
#         (marginal_gain[0] + marginal_gain[1]) / total_gain
#         if total_gain > 1e-12
#         else 0.0
#     )

#     subset_labels = {
#         m: "{" + ",".join(str(i + 1) for i in best_fixed_subsets[m]) + "}"
#         for m in m_values
#     }
#     subset_box = "Selected fixed subsets\n" + "\n".join(
#         f"{m}: {subset_labels[m]}" for m in m_values
#     )

#     fig = plt.figure(figsize=(8.8, 4.5))
#     gs = fig.add_gridspec(1, 2, width_ratios=[1.0, 1.12], wspace=0.32)
#     ax1 = fig.add_subplot(gs[0, 0])
#     ax2 = fig.add_subplot(gs[0, 1])
...
#         color="tab:orange",
#     )
#     ax1.set_title("(a) Sensor-count trade-off")
...
#         ha="right",
#         va="top",
#         fontsize=max(FONT_SIZE_LEGEND - 1, 8),
#         bbox=dict(
#             boxstyle="round,pad=0.25", facecolor="white", alpha=0.9, linewidth=0.5
#         ),
#     )

#     bars = ax2.bar(np.arange(len(step_labels)), marginal_gain, width=0.72)
#     ax2.set_title("(b) Marginal gain along fixed subset chain")
#     ax2.set_xlabel("Sensor-count step")
#     ax2.set_ylabel("Delta median NMSE")
#     ax2.set_xticks(np.arange(len(step_labels)))
#     ax2.set_xticklabels(step_labels)
#     ax2.set_ylim(0.0, max(marginal_gain.max() * 1.25, 0.05))
#     ax2.grid(True, axis="y", alpha=0.25)

#     for rect, val in zip(bars, marginal_gain):
#         ax2.text(
#             rect.get_x() + rect.get_width() / 2.0,
#             rect.get_height() + ax2.get_ylim()[1] * 0.02,
#             f"{val:.3f}",
#             ha="center",
#             va="bottom",
...
#         },
#         "marginal_gain_median_nmse": {
#             step: float(v) for step, v in zip(step_labels, marginal_gain)
#         },
#         "gain_fraction_recovered": {
#             "2_sensors": float(recovered_2),
#             "3_sensors": float(recovered_3),
#         },
...
#             ].mean(),
#             "mean_ccc": metrics.groupby("topology")["mean_ccc"].mean(),
...
#     # Use SciencePlots for a professional conference paper appearance
#     import scienceplots
#     from cycler import cycler

#     plt.style.use(["science", "ieee", "no-latex"])

#     # Override size parameters and inject a beautiful blue & orange primary color palette
#     plt.rcParams.update(
#         {
#             "axes.prop_cycle": cycler(
#                 color=["#0072B2", "#E69F00", "#009E73", "#CC79A7", "#D55E00"]
#             ),  # Colorblind-safe blue/orange styling
...
"""
Regenerate the data-driven plots used in the IFAC paper
"Designing Pneumatic Reservoir Sensing: Topology, Robot Stiffness, and Sensor Count".

What this script regenerates
----------------------------
- Figure 2: performance comparison
- Figure 3: signal diversity example
- Figure 4: operating regime / baseline-pressure analysis
- Figure 5: sensor ablation (sealed/parallel topology)
- Per-trial and aggregate metric CSV files
...
This script supports both same-time estimation (``horizon_steps=0``) and
forward-shifted targets (``horizon_steps>0``). The manuscript rerun produced
for the revised paper uses the same-time estimator so that the reported
benchmark evaluates present-state angle observability from the pressure field.

Figure 2 panels (a) and (b) use the same common raw-sensor estimator
pipeline as the aggregate NMSE and MC panels so that the representative traces
are directly comparable to the reported statistics. All plot text is configured
with a Times New Roman-compatible serif stack for manuscript consistency.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Target shift in samples. Use 0 for same-time estimation.",
    )
    return parser.parse_args()
...
def align_supervised_problem(
    ...
    """Align tapped-delay states with current/future angle targets.

    horizon_steps=0 yields a same-time estimator.
    horizon_steps>0 yields a strictly forward-shifted target.
    """
    if horizon_steps < 0:
        raise ValueError("horizon_steps must be >= 0")

    X = windows[:, :, subset_idx].reshape(len(windows), -1)
    y = theta_deg[delay - 1 + horizon_steps :]
    t = time[delay - 1 + horizon_steps :]
    if horizon_steps > 0:
        X = X[:-horizon_steps]

    if len(X) != len(y):
        raise ValueError(
            f"Aligned feature/target lengths differ: len(X)={len(X)}, len(y)={len(y)}"
        )
    return X, y, t
...
def prc_prediction_custom(
    ...
    *,
    ...
    """Estimate theta with optional sensor low-pass and custom delay/alpha."""
    sensors = trial.sensors
    if lp_cutoff_hz is not None:
        sensors = lowpass_filter_sensors(sensors, trial.dt, lp_cutoff_hz)
    ...
    windows = build_tapped_windows(sensors, delay)
    subset_idx = np.array(subset, dtype=int)
    X, y, t_full = align_supervised_problem(
        windows, trial.theta_deg, trial.time, delay, horizon_steps, subset_idx
    )
    y_test, y_pred, idx = fit_ridge_predict(X, y, alpha=alpha)
    t = t_full[idx]
    return PredictionResult(
...
def build_prediction_cache(
    trial: TrialData, *, horizon_steps: int = DEFAULT_HORIZON_STEPS
) -> PredictionCache:
    subset_idx = np.array(ALL_POUCHS, dtype=int)
    X_full, y_full, _ = align_supervised_problem(
        trial.sensor_windows,
        trial.theta_deg,
        trial.time,
        DELAY,
...
def build_trial_table(
    trials: Sequence[TrialData], *, horizon_steps: int = DEFAULT_HORIZON_STEPS
) -> pd.DataFrame:
    rows = []
    for trial in trials:
        pred = prc_prediction(trial, subset=ALL_POUCHS, horizon_steps=horizon_steps)
        mean_corr, _ = compute_pairwise_corr_stats(trial.sensors)
        eig, ratio = pca_from_covariance(trial.sensors)
        rows.append(
            {
...
def make_fig2(
    ...
    *,
    ...
    # Representative traces use the same common estimator pipeline as the
    # aggregate metrics reported elsewhere in the manuscript.
    pred_parallel = prc_prediction(
        example_parallel,
        subset=ALL_POUCHS,
        horizon_steps=horizon_steps,
    )
    pred_coupled = prc_prediction(
        example_coupled,
        subset=ALL_POUCHS,
        horizon_steps=horizon_steps,
    )
    ...
    fs_p = 1.0 / example_parallel.dt
    b_p, a_p = butter(3, 0.5 / (0.5 * fs_p), btype="low")
    y_pred_p_filt = filtfilt(b_p, a_p, pred_parallel.y_pred)

    fs_c = 1.0 / example_coupled.dt
    b_c, a_c = butter(3, 1.0 / (0.5 * fs_c), btype="low")
    y_pred_c_filt = filtfilt(b_c, a_c, pred_coupled.y_pred)
...
def make_fig4(
    ...
    topo_colors = {"coupled": "#0072B2", "parallel": "#E69F00"}
    waveform_markers = {"axial": "o", "circular": "s", "triangular": "^"}
    for _, row in metrics.iterrows():
        ax1.scatter(
            row["theta_std_deg"],
            row["nmse"],
            color=topo_colors[row["topology"]],
            marker=waveform_markers[row["waveform"]],
            s=34,
        )
    ax1.set_title("(a) Operating regime")
    ax1.set_xlabel(r"$\theta_{\mathrm{std}}$ (deg)")
    ...
    topo_handles = [
        Line2D(
            ...
            marker="o",
            color="w",
            markerfacecolor=topo_colors[topo],
            markeredgecolor=topo_colors[topo],
            markersize=6,
            linestyle="None",
    ...
    wf_handles = [
        ...
        for wf in ["axial", "circular", "triangular"]
    ]
    leg_topo = ax1.legend(
        handles=topo_handles,
        loc="upper left",
        frameon=True,
        framealpha=0.9,
        title="Topology",
    )
    ax1.add_artist(leg_topo)
    ax1.legend(
        handles=wf_handles,
        loc="lower left",
        frameon=True,
    ...
    ax2.set_xlabel("Baseline pressure $P_0$ (PSI)")
    ax2.legend(loc="best", frameon=True, framealpha=0.9)
    ...
    ax3.set_title("(c) Median NMSE vs baseline pressure $P_0$")
    ax3.set_xlabel("Baseline pressure $P_0$ (PSI)")
    ax3.set_ylabel("NMSE")
    ax3.legend(loc="best", frameon=True, framealpha=0.9)
    ...
    for topo in ["coupled", "parallel"]:
        ...
        for pre in [1, 2, 3]:
            ...
            ys.append(med)
        ax4.plot(xs, ys, marker="o", label=topo.capitalize())
    ax4.set_title("(d) Mean CCC vs baseline pressure $P_0$")
    ax4.set_xlabel("Baseline pressure $P_0$ (PSI)")
    ax4.set_ylabel("Mean CCC")
    ax4.legend(loc="best", frameon=True, framealpha=0.9)
...
def make_fig5(
    ...
    *,
    ...
    caches = {
        trial.meta.label: build_prediction_cache(trial, horizon_steps=horizon_steps)
        for trial in parallel_trials
    }
    ...
    subset_box = "Best subset by sensor count\n" + "\n".join(
        f"{m}: {subset_labels[m]}" for m in m_values
    )
    ...
    ax1.set_title("(a) Sensor-count trade-off")
    ax1.set_xlabel("Number of instrumented sensors")
    ax1.set_ylabel("NMSE (median across trials)")
    ax1.set_xticks(ms)
    ax1.set_ylim(0.08, 1.02)
    ax1.grid(True, alpha=0.25)
    ...
    bars = ax2.bar(np.arange(len(step_labels)), marginal_gain, width=0.72)
    ax2.set_title("(b) Best NMSE change by sensor count")
    ax2.set_xlabel("Sensor-count step")
    ax2.set_ylabel("Change in best median NMSE")
    ax2.set_xticks(np.arange(len(step_labels)))
    ax2.set_xticklabels(step_labels)
    ymin = min(-0.05, float(marginal_gain.min()) * 1.35)
    ymax = max(0.05, float(marginal_gain.max()) * 1.25)
    ...
    for rect, val in zip(bars, marginal_gain):
        ...
        y_text = rect.get_height() + offset if val >= 0 else rect.get_height() - offset
...
def main() -> None:
    ...
    # Override size parameters and inject a beautiful colorblind-safe palette
    plt.rcParams.update(
        {
            "axes.prop_cycle": cycler(
                color=["#0072B2", "#E69F00", "#009E73", "#CC79A7", "#D55E00"]
    ...
    try:
        ...
        trials = load_all_trials(data_root)
        metrics = build_trial_table(trials, horizon_steps=args.horizon_steps)
        metrics.to_csv(out_dir / "trial_metrics.csv", index=False)

        make_fig2(trials, metrics, out_dir, args.dpi, horizon_steps=args.horizon_steps)
        make_fig3(trials, out_dir, args.dpi)
        make_fig4(metrics, out_dir, args.dpi, args.panel4b_mode)
        fig5_summary = make_fig5(
            trials, out_dir, args.dpi, horizon_steps=args.horizon_steps
        )
        write_summary_files(metrics, fig5_summary, out_dir)
