...
desired_cols = ["Desired_pressure_segment_1", "Desired_pressure_segment_2", 
                "Desired_pressure_segment_3", "Desired_pressure_segment_4"]
measured_cols = [f"Measured_pressure_Segment_1_pouch_{i}" for i in range(1, 6)] + \
...
if tmax <= 20:
    ...
    df_working = df_working[(df_working["t_rel"] >= trim_duration) & 
                            (df_working["t_rel"] <= (tmax - trim_duration))].reset_index(drop=True)
else:
    df_working = df_working[(df_working["t_rel"] >= 10.0) & 
                            (df_working["t_rel"] <= (tmax - 10.0))].reset_index(drop=True)
...
titles = ["Segment 1 emulation (test)", "Segment 2 emulation (test)", 
          "Segment 3 emulation (test)", "Segment 4 emulation (test)"]
...
for i in range(4):
    axes[i].plot(t_plot, Y_test[:, i], label=f"Seg{i+1} true", 
                linewidth=2.5, color=true_color, alpha=0.9)
    axes[i].plot(t_plot, Yhat_test[:, i], "--", label=f"Seg{i+1} pred", 
                linewidth=2.5, color=pred_color, alpha=0.9)
    axes[i].set_ylabel("Pressure", fontsize=16, fontweight='bold')
    axes[i].set_title(titles[i], fontsize=12, fontweight='bold')
    axes[i].grid(True, alpha=0.3)
    axes[i].legend(loc="upper right", frameon=True, fontsize=18)
...
color_palettes = [
    ['#E41A1C', '#377EB8', '#4DAF4A', '#984EA3', '#FF7F00'],  # Segment 1: Bold primary colors
...
group_titles = ["Segment 1 Pouches (s1–s5)", "Segment 2 Pouches (s6–s10)", 
                "Segments 3 & 4 (s11–s12)"]

for ax, g, title, colors in zip(axes2, groups, group_titles, color_palettes):
    for k, color in zip(g, colors):
        ax.plot(t_plot, test[f"s{k}"].values, label=f"s{k}", 
               linewidth=2.2, color=color, alpha=0.9)
